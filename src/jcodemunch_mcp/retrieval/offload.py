...
is NOT registered as an MCP tool (the catalog moratorium in force since
v1.108.218 admits no new top-level actions). Callers build an
...
#: count as unambiguous. Reuses ``confidence.compute_confidence``'s ``gap``
#: component rather than deriving a second margin: two code paths settling one
#: question is a defect class we have paid for before.
...
class EvidenceShape:
    ...
    #: 0-1 top-1 vs top-2 dominance, from ``confidence.compute_confidence``'s
    #: ``gap`` component. Meaningful only when ``retrieval_mode == "ranked"``.
    ...
    #: e.g. ``{"tool": "check_references", "args": {...}}``. Optional, but a
...
ENV_GATE = "JCODEMUNCH_OFFLOADABLE"
