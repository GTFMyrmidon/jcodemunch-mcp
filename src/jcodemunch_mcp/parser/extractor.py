...
# `</script\s*>`: an end tag may carry whitespace before `>` (`</script >` is valid HTML), and a
# block ended that way would otherwise swallow everything to the NEXT `</script>` (CodeQL
# py/bad-tag-filter, code-scanning alerts 13 and 14).
_RAZOR_SCRIPT_RE = re.compile(r"<script\b([^>]*)>(.*?)</script\s*>", re.IGNORECASE | re.DOTALL)
_RAZOR_STYLE_RE = re.compile(r"<style\b([^>]*)>(.*?)</style\s*>", re.IGNORECASE | re.DOTALL)
...
_ASTRO_SCRIPT_RE = re.compile(r"<script\b([^>]*)>(.*?)</script\s*>", re.IGNORECASE | re.DOTALL)  # `\s*`: see the Razor twin
_ASTRO_STYLE_RE = re.compile(r"<style\b([^>]*)>(.*?)</style\s*>", re.IGNORECASE | re.DOTALL)
