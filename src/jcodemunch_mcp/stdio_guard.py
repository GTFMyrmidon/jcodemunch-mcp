...
to that stream breaks a response. jcodemunch already carries scar tissue from
this: `run_stdio_server`'s handshake watchdog exists because a paying client on
Codex/rmcp waited 5h+ for a frame that was never coming, after `uvx`
...
``tools/embed_repo.py`` constructs a ``SentenceTransformer`` inside a tool call,
so a first embed on a machine without the model cached downloads it mid-request
and its native progress output goes straight at the JSON-RPC stream.
...
⚠ **This does not fix the uvx case and the watchdog stays.** Chatter written by
a launcher *before* our process starts is already in the pipe; nothing we do
after exec can retract it. This closes everything written from our own process
onward, which is a different half of the same problem.
"""
