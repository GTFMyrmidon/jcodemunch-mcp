...
jCodeMunch is an MCP server, so it plugs into any MCP-capable client. `jcodemunch-mcp init` auto-configures the common ones (Claude Code, Claude Desktop, Cursor, Windsurf, Continue, Codex CLI, opencode). This page collects the tested configurations for everything else, plus the client-specific gotchas.
