...
## Current State
- **Version:** 1.108.261. **A disclosure sentence that names every field.** **#424 DECIDED and CLOSED by jjg: the anonymous savings record does NOT carry a config dimension** - no sender change, no new field, and the disclosure edits sequenced behind that decision correctly become NO edits. ⚠⚠ **Verifying that rather than assuming it found one sentence already incomplete**: `SECURITY.md` said "**only** sends an integer delta plus an anonymous UUID" while the payload is `{delta, total, anon_id}` - the lifetime `total` was NEVER NAMED, in a sentence containing the word "only". Same CATEGORY of number (an incompleteness, not an undisclosed kind of data), and the other two sites were already correct because they say "counts"/"counters" PLURAL. ⚠ **It still matters**: that paragraph is what a security reviewer reads, "only" is a promise about the COMPLETE set, and this project was quarantined once over exactly the gap between what a package does and what its docs say. **The decision to add nothing is what makes the existing sentence the FINAL word, so it has to be exactly right.** New text names all three fields + "never ... any configuration value", carrying the #424 decision into the disclosure itself. ⚠ **Enforced, not just corrected** ([[feedback_a_convention_needs_a_test_not_a_habit]]): one test pins the payload at exactly 3 known fields and NAMES SECURITY.md in its failure message so code and disclosure cannot separate; another requires the sentence to name delta/lifetime total/UUID/no-config-value and **FAILS against the old wording**, which is how the fix was verified. `test_the_telemetry_payload_carries_no_surface_field` now records #424 as DECIDED, so a future session cannot re-derive the field without deliberately deleting a test that explains why not. **No behaviour change; docs + tests only.**
