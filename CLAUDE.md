...
## Current State
- **Version:** 1.108.201 - **A measurement that was never written down.** F-15 Anchor P0 collection gap. v1.108.167 shipped `redelivery_rate` on the yield block and THREW IT AWAY every session: `_delivered` is process memory and `_yield_locked()` had exactly ONE caller (`_build_stats_locked`, serving `get_session_stats`). Nothing persisted it, so the F-15 P2 gate has been waiting since 2026-07-24 on a number NO SESSION COULD CONTRIBUTE TO, and `benchmarks/anchor/` never existed. ⚠ **Verified 2026-07-30: `telemetry.db` held 0 `tool_calls` rows and 1 `ranking_event` (April) - and `ranking_events` could not have answered it anyway (no session id; written only by `search_symbols`/`get_ranked_context`/`plan_turn`, NEVER the source-dump tools where the bytes are).** New `session_yield` table: ONE row per process, **UPSERTED on each flush, not appended** (a SIGKILLed server still leaves its latest counts; no session counted twice). ⚠ **Stores RAW COUNTS, not the rate - pooling numerators/denominators is the only correct aggregate; averaging per-session rates weights a 3-delivery session like a 300-delivery one and CHANGES THE VERDICT.** ⚠ **Opt-in behind the EXISTING `perf_telemetry_enabled` (off by default) - no new switch, no new default-on surface - and README-disclosed per the standing rule.** `benchmarks/anchor/measure.py` pools it and applies the pre-registered rule (<10% kills P2 / 10-25% opt-in / >25% default-on live), reporting `insufficient_data` below 200 deliveries over 5 sessions because **too little data must NEVER read as "under 10%, kill it."** ⚠ **NO synthetic corpus, deliberately: redelivery measures whether an agent re-fetches what it holds, so a scripted session would CHOOSE the answer to a rule pre-registered so it could not be.** Tests: `tests/test_v1_108_201.py` (9). **The number still does not exist; it is now merely collectable.**
- **Prior (1.108.200):** - **A staleness warning that fires on nearly every read protects nothing.** SCIP P3: staleness now requires a COVERED FILE to have changed, not just a moved repo-global HEAD; fails toward stale on missing provenance; zero consumer edits (all five read through one function); `digest` gained a SCIP section, absent when no ingest so other digests stay byte-identical. Docs: `SCIP.md`.
