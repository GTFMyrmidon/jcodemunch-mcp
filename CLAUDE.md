...
- **Prior (1.108.243):** **a rebuild the caller did not ask for is now a FIELD, not a sentence** (#413, @LuigiNicaPRO). `load_index()` returns `None` on **SEVEN** paths (`sqlite_store.py:1485/1496/1509/1519/1525/1528/1552`) and `index_folder.py`/`index_repo.py` collapsed all seven into ONE warning naming ONE of them — so a **corrupt** index was reported as a downgrade with "delete `~/.code-index/`" as the remedy, and an **empty-meta** index took the same full rebuild with **NO message at all**. ⚠⚠ **The substitution erased its own evidence**: the rebuild re-stamps `meta.index_version`, so after the call the DB is indistinguishable from one that was always healthy and a caller who did not capture `warnings[]` AT THE MOMENT OF THE CALL cannot learn it happened — which is why no post-hoc check could ever have caught this. Fix routes the WRITE side through `inspect_index()`, the discriminator **PR #291 already built for the READ side** (`resolve_repo`/`_utils`/`get_redaction_log`/`get_runtime_coverage` used it; the write side never did). Every non-error return of both tools now carries `requested_incremental` + `performed_incremental`, plus `rebuild_reason` when an unreadable index forced it. ⚠ `inspect_index` disagreeing with `load_index` (the two delete-during-load races) yields `unloadable_unknown` — unknown is a THIRD answer, never a guessed cause. ⚠ The one cause that IS a downgrade keeps the "newer version" wording (a pre-existing `test_tools.py` assertion whose docstring named exactly that subject, so the CODE was re-grounded, not the test) with the remedy narrowed to that one repo's index. ⚠ Verified at the MCP **entry point** (`server.call_tool`), not at the layer edited — the three fields survive the encoder to the wire. ⚠⚠ **Do NOT run `uv lock` to bump the version on uv 0.12.1**: it rewrites torch's CUDA-extra markers (drops every `sys_platform` guard, 53 lines) as resolver preference-drift, and `uv lock --check` passes either way, so the rewrite is invisible to CI. Hand-edit the one `version =` line. Tests: `test_unloadable_index_rebuild_disclosure.py` (7), incl. non-vacuity — a healthy incremental must be able to say nothing was substituted, and a first index must not invent a cause.
- **Prior (1.108.242):** **documentation restructure.** README split into `CAPABILITIES.md` (capability tour) and `CLIENTS.md` (per-client setup); 1223 -> 213 lines. ⚠ **Tool counts were wrong in three places and are corrected against the shipped wheel**: `list_tools()` returns **91** at the default profile (documented as 62), core is **17** (16), standard is **79** (51). ⚠ Counter figures refreshed from live telemetry (645B tokens, 95,269 installs) — pull them from the counter, never from a doc. ⚠⚠ **Moving "Background behavior, fully disclosed" out of README breaks the disclosure deep link jcodemunch.com points at**, so README keeps an explicit anchor of that id and the site now targets SECURITY.md. ⚠ `test_docs_config_parity` parsed `cells[2]` of EVERY 3-column table and let later rows win, so a savings table clobbered real defaults; it now reads only the Default column, which is what its docstring always claimed. Docs only, no code change.
- **Older releases (1.108.241 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **INDEX_VERSION:** 17
- **Tests:** 6994 passed, 7 skipped, **0 failed** (1.108.244, full local run, 16m02s). ⚠ **The "KNOWN 12 local-ONNX `test_semantic_search` env failures" are GONE** — .207's autouse `no_local_onnx` fixture fixed them, so a local run is now fully green and **any** red is a real signal. Do not carry that 12-failure allowance forward; it papered over a real failure once already (.197 had one hiding inside it). ⚠ **Still do not eyeball the COUNT** — diff the FAILED names against the same tree with your changes stashed; for .199 and .205 that diff was empty, and for .209 the failure set was empty outright, which is the one case that needs no baseline. ⚠ **Stashing is the wrong tool when the change is already committed and pushed** — for .205 the comparison ran in a throwaway `git worktree add --detach <pre-release-sha>`, which also survives a concurrent writer in the main tree.
- **Python:** >=3.10
