...
## Current State
- **Version:** 1.108.272. **A column recorded on the wrong exit is not a measurement.** [#440](https://github.com/jgravelle/jcodemunch-mcp/issues/440) (@rknighton). Both non-fusion `search_symbols` exits built a score-only `[{"score": s}]` ledger input and handed it to `extract_ledger_features`, which reads `identity`/`identity_match` — so `identity_hit` was recorded **0 on every row whatever the identity channel found**, by construction rather than by measurement. His repro runs one query down two exits: default `identity_hit=0` on an exact symbol-name match, `search_symbols_fusion` `1` for the same query and the same top result. ⚠⚠ **The defect is less interesting than what HID it: the v1.108.187 fusion fix left a comment asserting the non-fusion paths already carried `identity`, and that exoneration was FALSE.** They carried it nowhere the reader looks — the lexical path folds it into the BM25 total inside `_bm25_score`, and `debug=True` nests it under `score_breakdown`. **Near the reader is not where the reader looks**, and a comment claiming coverage READS as coverage. The old comment is corrected in place, because leaving it standing preserves the trap for the third instance ([[feedback_a_fix_comment_is_not_evidence_about_its_siblings]]). ⚠ **`_ledger_identity_rows` RECOMPUTES identity rather than threading it out of scoring, deliberately** — `semantic_only` skips the channel entirely (`idn = 0.0`), so reusing the scorer's value would keep recording a default dressed as a measurement, i.e. the very thing being fixed. Pure function of name/id + query, top-3 only, three string comparisons. ⚠⚠ **The rows are fed to the LEDGER only, NEVER to `attach_confidence`** — `compute_confidence` sniffs the same key when `has_identity_match` is None and scores 1.0 known-true / 0.7 unknown, so sharing one input would move the published confidence of every non-fusion search. **Recording a column must not move a number callers already read**; the test asserts the published value is unchanged AND first asserts the leak WOULD move it, or it proves nothing. ⚠⚠ **The history is NOT repairable and no heuristic was invented.** Pre-fix rows always passed `top1_score` and only omitted the identity key, so they are indistinguishable from an honest post-fix 0 — same reasoning `ledger_trust` already applied to `search_symbols_fusion`. **But `search_symbols` is the highest-volume producer in the ledger, so the contaminated share is FAR larger than the fusion case that predicate was written for: do NOT read "the fusion rows are handled" as "the `identity_hit` column is clean".** Recency window is the only remedy. Two consumers were affected and both stay contaminated for old rows: `analyze_perf.identity_hits` undercounted, and `regret`'s vocabulary-gap signal (`not identity_hit and semantic_used`) had **both halves satisfied by defect** on the semantic exit, which passes `semantic_used=True` literally — above the confidence floor and recurrence threshold it was reporting a gap it had not tested, into user-visible `suggest_corrections` patches. ⚠ **Severity was checked UPWARD per the standing note on this reporter and came back NEUTRAL — third time running** (#411, #412, #440); the WeightTuner is not a third consumer (`tuning.py:197` enumerates column 10 in a comment and consumes column 9). Treat his direction as genuinely unknown, not expected-low. `tests/test_v1_108_272.py` (9), non-vacuity proven by reverting **only the two call sites** on the same tree — 1 fails pre-fix, the 8 passing both sides are unit-level and no-heuristic controls. ⚠ Stashing the whole file only produced a collection error, which proves nothing. ⚠⚠ **[#435](https://github.com/jgravelle/jcodemunch-mcp/issues/435)'s `next` exemption is now UNBLOCKED and was deliberately NOT taken in this release** — PR #433 merged 2026-08-11 23:51 UTC, so the close condition below is MET. Held out because it is a different subsystem needing its own reachability-seed tests, and churning the exact lines a 12-hour-old merge touched belongs on its own legible commit. **Next piece of work.** ⚠ #441 (untruncated-count column) and #442 (per-event schema replay) are OPEN from the same @rknighton batch and the same write path; #441 is a migration on the table this release just touched, so it was kept separate on purpose.
