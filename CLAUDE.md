...
## Current State
- **Version:** 1.108.213 — **the Counter stopped ranking on the word "me".** `_idf_weights` computes idf over description PROSE, so a pronoun (absent from every description) drew the **highest weight in the query**, and `score_action` then spent that weight on a SUBSTRING match against the tool NAME — a different corpus. Two-char noise matched a third of the catalog inside snake_case names. ⚠⚠ **Measured: on "draw me a diagram of that", `check_rena(me)_safe` scored 19.3 and BEAT `render_diagram`'s 16.5 for the actual word "diagram"**; `get_runti(me)_coverage` + `find_i(mple)mentations` also placed above it on that one token. Even "a" scored 2.5. ⚠ **Second defect underneath: the fragment branch (+4.0) outranked the whole-word branch (+3.0)**, so a hit INSIDE a word beat an exact word hit — and since every token is trivially a substring of its own name, the whole-word branch was **unreachable dead code**. Fix: `_STOPWORDS` filter via new `_query_tokens` (`_tokens` stays raw for the leakage metric), `_MIN_SUBSTRING_LEN = 4` floor, whole-word promoted above fragment. **menu@3 44.1 → 61.0, menu@1 23.7 → 40.7, route@3 49.2 → 59.3, route@1 30.5 → 42.4**; 26 menu queries improved vs 7 regressed. **Closes the `menu` gap disclosed in .212** (transform now 100% on BOTH paths). ⚠ **Three hit→miss "regressions" have ZERO exact token overlap with their queries** — their old ranks 2/3/4 were stopword accidents; losing a fake hit is the fix working. ⚠ **KNOWN RESIDUAL, now visible to the harness: NO STEMMING** — "risky" cannot reach `get_pr_risk_profile`, "concentrated" cannot reach `get_architecture_metrics`; ranking cannot rescue a query sharing no token with its target. That is the next lever. 4 regression tests in `tests/test_counter.py`.
- **Prior (1.108.212):** — **`route()` can reach the three transform actions it could never propose.** `render_diagram` / `plan_refactoring` / `assemble_task_context` consume another tool's OUTPUT rather than querying the index, so the words a user types for them appear nowhere in the catalog text the lexical fallback ranks over, and no curated rule covered them. **Measured route recall for that group was 0%** — capability shipped, route to it did not. Three `_INTENT_RULES` entries + broader task-setup phrasing: **transform 0% → 100% at rank 1, zero regressions**. ⚠ **Placement is load-bearing**: the transform block sits LATE so a specific data-fetch intent still wins primary ("visualize the call graph" leads with `get_call_hierarchy`, because `render_diagram` consumes that output), but ABOVE the generic `plan` rule that would otherwise claim every refactor-plan request. Both directions pinned in `tests/test_counter.py`; the inverted-pair bug was caught BY writing the guard test. **New `benchmarks/route_recall/`**: measures `menu`/`route` recall@k separately, per-group, rule-vs-fallback, and refuses to report when a corpus target is absent from the live catalog. ⚠⚠ **Every query carries a measured LEAKAGE score** — it caught a real 24.5-point inflation, so quote the CLEAN subset. ⚠ Single-shot: measures the retriever, NOT the agent loop; never read it as a failure rate.
- **Prior (1.108.211):** — **"this repo has no embeddings" and "not THESE candidates" stop looking identical** (#398 follow-on, @rknighton reviewing the code .210 shipped hours earlier). Dropping `count()` was right and is NOT undone - but `count()` was repo-wide and the old code branched on `count() > 0` BEFORE narrowing to candidates, so deriving everything from `len(get_many(cand_ids)) >= 2` collapsed two states. ⚠⚠ **The collapse was not merely unsurfaced, it was WRONG OUT LOUD**: both states emitted one note telling the caller to run `embed_repo`, which is false advice for an already-embedded repo (running it again on the same symbols changes nothing). That note PRE-DATES .210 - the old `count()` guard never surfaced the difference either - so this fixes a live misdirection, not a pre-positioned signal. New `EmbeddingStore.has_any()`: `SELECT 1 ... LIMIT 1` on the same `mode=ro&immutable=1` conn, no scan/no write/no sidecars, and it runs ONLY when zero candidate vectors came back (one fetched vector already proves the repo is embedded). ⚠ **TRI-STATE**: `False` is reserved for file-absent or table-absent; a locked file / corrupt page / permission error answers `None`, because `False` there is a guess about a repo we could not read (same rule as .209 freshness and .180 `repo_freshness`). Structural responses carry `semantic_state` (`repo_not_embedded`/`candidates_not_embedded`/`unknown`/`not_applicable`) + a per-state note; **hybrid responses are byte-identical to .210** (`mode` already carries it, and that's the most-called shape). ⚠ **THREE exits can return a structural answer and TWO disclosed nothing** - the zero-edge exit returned `mode: "structural"` with no note at all, so a zero-cluster answer from an un-embedded repo read exactly like one from an embedded repo; all three now route through `_structural_disclosure()`, and the too-few-candidates exit reports `not_applicable` because it never consults the store ([[feedback_check_all_code_paths_for_new_features]]). **#398 Arc decisions recorded in `ROADMAP.md` the same day**: Arc 2 must NEVER use `immutable=1` (the un-checkpointed-WAL trade-off is channel-specific - a missed vector weakens a RANKING, a missed ROW is a false absence); Arc 3 byte budget is primary but does **NOT** subsume `JCODEMUNCH_INDEX_CACHE_TTL` (a byte budget is per-PROCESS, #375's leak was 25+ processes - a cap bounds growth, only idle eviction reclaims the floor); Arc 4's gate is now a THREE-bucket breadth split with Django authoritative and thresholds named BEFORE the run. Tests: `test_v1_108_211.py` (16).
- **Older releases (1.108.210 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **INDEX_VERSION:** 17
