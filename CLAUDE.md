...
- **Prior (1.108.228):** **ranking ties break on the symbol id, not on insertion order** (#398 Arc 4 / #403). `ROADMAP.md` has said since 08-02 that a `(score, symbol_id)` key **"is required regardless and ships FIRST, alone"**; `fuse()` always used it, the four channel builders and the semantic scorer did NOT (stable sort on score → ties broke on the order SQLite handed rows back). ⚠⚠ **.223 turned this from correct-in-principle into URGENT and we missed it at the time**: that release gave the semantic scorer TWO lanes (numpy float32 matmul / pure-Python float64), scores differ ~1e-7, and on a deliberately near-tied 4,000-vector corpus they **DISAGREED AT RANK 0** — two installs of the SAME VERSION ranking differently based only on whether numpy was importable. ⚠ **That corpus is SYNTHETIC and maximally homogeneous; the counterweight belongs in the same breath** — @rknighton measured **ZERO** float32/float64 boundary disagreements on REAL embeddings (Django/FastAPI/jcm). Hazard real in principle, does not fire in practice on those corpora. A total order does NOT remove the float difference; it removes the OTHER one. **#403 VERDICT: Arc 4 gate CLEARED** (Django 0.1591% / FastAPI 0.1809% / jcm 0.2686% vs a 10% ceiling, 0 boundary disagreements vs 0.5%). ⚠ **We checked the gate's own PRECONDITION and it had NOT been met** — the tie-break key had not shipped when the measurement ran, so bucket (1) was never collapsed; that makes breadth an UPPER bound so a pass holds a fortiori, but it would have invalidated a FAIL. ⚠ Did NOT re-run the harness; evidence repo still has **no LICENSE** (read/re-run, never vendor). **Lane 1 already shipped as .223** (answering #399, not as Arc 4 work) so bucket (3) now describes PRODUCTION code; **lane 3 parked on a changed premise** — it exists to make an APPROXIMATE scorer safe and the exact one is now 2.9 ms warm. Tests: `test_v1_108_228.py` (10).
- **Prior (1.108.227):** **`git_sha` verification answers the question it was asked** (@rknighton, #402 — filed WITHOUT a patch, routing left to us). The index is built from the WORKING TREE at one moment and verified against LIVE HEAD at another, so a cache that is a byte-exact record of what it indexed reported `git_sha_mismatch`. ⚠⚠ **THE MEASUREMENT THAT REDIRECTED THE FIX: the `rev` parameter the report proposed does NOT fix the case the report REPRODUCES.** A tree DIRTY at index time has `index.git_head == live HEAD`, so passing the recorded revision changes NOTHING — **no commit anywhere contains those bytes, because they were never committed.** Shipping `rev` alone would have LOOKED like a fix. Three cases that were ALL `git_sha_mismatch` are now three answers: **(A)** commit landed after indexing and changed this file → `git_sha_match` (this is what `rev=index.git_head` fixes, shipped as proposed); **(B)** dirty at index time → new **`git_sha_uncommitted`**, reached by asking a SECOND question instead of inventing a third revision — *does the cache match the WORKING TREE?* If yes the cache is faithful and the commit is simply behind it; **(C)** matches neither → still `git_sha_mismatch`. ⚠ **`git_sha_uncommitted` is a SOFTER verdict so the guard is load-bearing**: a cache matching the tree no better than the commit still mismatches **even on a dirty tree where the soft verdict is available**, and .224's truncation guard survives because both comparisons run through ONE `_slice_matches` (two rules that can drift apart is exactly what made #400/#401 possible). **New field `git_sha_rev`** = the revision actually compared against, reported ALWAYS not just on fallback — the recorded rev can be rebased/force-pushed away, and falling back to HEAD without saying so would be a silent change of subject. `_verify_against_git_sha` now returns `(status, rev_used)`. `verify_against="cache"` untouched, test-asserted. ⚠⚠ **`verify_against` moved into `_COMPACT_STRIP_PARAMS` and the MEASUREMENT is why: core_compact had FOUR tokens of headroom**, so any core-tier description gaining a clause breaks `test_live_core_compact_under_4000_hard_ceiling`. **Trimming prose to fit is the WRONG instinct — it just makes the next person trim again**; the budget is designed to take a stripped param instead. Hidden ≠ removed: `_DECLARED_ARG_KEYS` is snapshotted BEFORE the strip so a hidden-but-honoured param can't read as a caller mistake (the .177 class), and a test calls the tool with the stripped param to prove it. Tests: `test_v1_108_227.py` (17).
- **Older releases (1.108.226 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **INDEX_VERSION:** 17
- **Tests:** 6680 passed, 7 skipped, **0 failed** (1.108.229, full local run, 14m21s). ⚠ **The "KNOWN 12 local-ONNX `test_semantic_search` env failures" are GONE** — .207's autouse `no_local_onnx` fixture fixed them, so a local run is now fully green and **any** red is a real signal. Do not carry that 12-failure allowance forward; it papered over a real failure once already (.197 had one hiding inside it). ⚠ **Still do not eyeball the COUNT** — diff the FAILED names against the same tree with your changes stashed; for .199 and .205 that diff was empty, and for .209 the failure set was empty outright, which is the one case that needs no baseline. ⚠ **Stashing is the wrong tool when the change is already committed and pushed** — for .205 the comparison ran in a throwaway `git worktree add --detach <pre-release-sha>`, which also survives a concurrent writer in the main tree.
- **Python:** >=3.10
