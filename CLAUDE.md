...
- **Prior (1.108.168):** — **A rebuild underneath a scan cannot prove absence — the 5th absence-evidence refusal rule (fixes a real hole in v1.108.166, found 2 days after shipping it).** v1.108.166's four rules (only `absent`; not `low_confidence`/`degraded`; not stale; not truncated) had **no rule for an index being REWRITTEN while the scan reads it**. Structural cause: `channels.index` is fed by `_probe.repo_is_stale` (stored git SHA vs live HEAD), which is **blind to a reindex of an UNCHANGED tree** — a watcher rebuild after an uncommitted edit, or the #375 silent/stalled index run. Such a scan reported `index:"fresh"`, reached `absent`, and `note_absence` minted a citable `absent:<sha>` ref: **the contract that exists to make "we looked and it's not there" auditable could attest it over a half-written index.** Fix: zero results + detected rewrite ⇒ `degraded`, so **the 5th rule falls out of the existing "only `absent` proves absence" check — no new rule to keep in sync**; `absence_refusal` gains a branch BEFORE the generic state check so the reason names the rebuild instead of saying "verdict was 'degraded'". `channels.index` gains **`"rebuilding"`, disclosed on EVERY state** (an `ok` caller deserves to know the index moved under it) — only the absence CLAIM is refused, results still return because those symbols really were in the index. ⚠ **`schemas/retrieval-verdict.schema.json` enumerates `index` under `additionalProperties:false`** — "rebuilding" MUST be published there or every mid-rebuild response fails validation against our own contract (`test_provenance.py` jsonschema-validates live responses); pinned by a new test. **Detection is a FILESYSTEM signal, deliberately NOT `reindex_state`** — that module is in-memory/per-process so a server answering a search CANNOT see a reindex run by a separate `watch-all` service (the common deployment, and #375's shape); routing through it would ship a rule that reads enforced and isn't (the jdata-P3 mistake). `load_index` stamps `_db_path` + `_loaded_mtime_ns` at all 3 return points (`_stamp_load_provenance`); `retrieval.verdict.index_changed_since_load` re-stats via the pre-existing `_db_mtime_ns` (max of .db/.db-wal, already used for LRU invalidation). **Unknown ≠ changed** (unstamped index → False, never degrade every verdict). Downgrade scoped to the zero-result path; check is 2 stats. Byte-identical when nothing rebuilds; legacy `negative_evidence` untouched. NO tool-count/INDEX_VERSION change; ruff clean. New `tests/test_v1_108_168.py` (18). Origin: 2-day competitor sweep — a rival shipped "report in-flight reindex instead of an empty, stale graph" in their STATUS tool; the renovation is putting it in the RETRIEVAL VERDICT (our lane). ⚠ **SUITE PARITY OWED: jdoc + jdata ship the same absence contract and the same 4 rules; their 5th is NOT written.**
- **Older releases (1.108.167 and earlier):** see `CHANGELOG.md` — the authoritative version history (411 entries, 1.0.0 onward).
- **INDEX_VERSION:** 17
- **Tests:** 5680 passed, 7 skipped (1.108.170) + the KNOWN 12 local-ONNX `test_semantic_search` env failures (green in CI on all 4 ubuntu jobs — never read them as a regression)
- **Python:** >=3.10
