...
## Current State
- **Version:** 1.108.259. **A re-index you can schedule** (#395, @dkiaulakis). We ask users to re-index constantly - issue replies, staleness warnings, the `stale_index` verdict channel, every `INDEX_VERSION`/`PARSER_GENERATION` bump - and all of it assumes re-indexing is cheap enough to run ON REQUEST. On a fleet sharing a store it is a scheduled maintenance event, so **our standard advice was unactionable for exactly the users with the most code**. New `refresh` subcommand: same work in bounded resumable slices (`--max-seconds` / `--max-files` / `--pause-ms`), persisting a cursor so N short windows converge the same as one long one. ⚠⚠ **`paths=[...]` was NOT already enough, contrary to the issue's own framing**: `detect_changes_with_mtimes` compares content HASHES, so a subset refresh over unchanged files correctly reports "No changes detected" - right for an edit, useless for a `PARSER_GENERATION` upgrade where the bytes are identical and the stored SYMBOLS are what is wrong. Hence `force_reparse`, valid **only alongside `paths`** (forcing without a list re-parses the corpus in one call, i.e. the unbounded event this exists to avoid). ⚠⚠ **The defect this nearly shipped with**: `index_folder` escalates a stale generation to a FULL re-parse, so every "bounded" slice ran the ENTIRE maintenance event - **measured at four full re-parses where four bounded slices were requested**, making a campaign of N slices cost N full re-indexes, strictly WORSE than the one event it replaces. A forced subset is now exempt and **nothing else is**, because no other caller can finish the job; **6 tests fail if the exemption is removed**. ⚠⚠ **Coverage is VERIFIED, not assumed** - `index_store.py` already stated the rule (an incremental save leaves the stamp alone because a partially re-parsed index must keep claiming the OLDER generation; a campaign that stamps early is that bug with a scheduler in front of it). So the stamp lands only after re-running discovery at the end: files added mid-campaign were never parsed by it, so they are APPENDED and the stamp DEFERRED (`corpus_drifted`); any batch error blocks it; `stamp_parser_generation` refuses to move BACKWARDS. ⚠ `--pause-ms` is the duty-cycle knob - **a budget bounds when a run ENDS, not what it costs while it runs** (Python cannot preempt a parse; same limit as `JCODEMUNCH_PARSE_BUDGET_SECONDS`). ⚠ AI summaries **OFF by default here**, unlike `index_folder`: a scheduled job must not bill a paid summarizer unasked. State written atomically; a corrupt state file starts fresh rather than stranding the operator. **Cost measured: 60 files in 3.8s (~63ms/file), ~50s for this 795-file repo**, documented in USER_GUIDE.md. Tests: `test_refresh_campaign.py` (32).
- **Prior (1.108.258):** **A config read that reads the config** (#426). `config.get()` returned the hardcoded DEFAULT for every env-mapped key until `load_config()` had run, so `JCODEMUNCH_MAX_FILE_SIZE=20000000` read back as 512000 and a caller could not tell "the value is 512000" from "I have no idea what the value is". ⚠ Not one key - `max_folder_files`/`max_index_files`/`staleness_days` behaved identically; the env mapping was right all along, it is only applied by `load_config()`, and `get()` reads `_GLOBAL_CONFIG`, `{}` until then. ⚠⚠ **Nothing was broken in production and it was fixed anyway**: `index`/`index-file`/`serve` all dispatch AFTER the shared `load_config()` in `main()`, so no shipped path resolved a limit unloaded (the #425 "not verified" note resolves as a HARNESS artifact). What forced it is that `server.py` already carried THREE comments of this shape on three different subcommands, each a real bug found SEPARATELY and fixed by adding one `load_config()` to one handler - **fixing rows one at a time is precisely why there is always a next row** (same shape as the `config --check` drift fixed in .250 and AGAIN in .255). `get()` now loads lazily so the ordering question stops existing. ⚠ **It does NOT displace the explicit calls** - `main()` re-runs `load_config()` after `_setup_logging()` on purpose so config warnings reach the configured log destination; only the LAZY path is conditional. ⚠ Gated on not-loaded **AND** empty via a separate `_CONFIG_LOADED` flag: emptiness alone re-reads the file per key for any state that legitimately resolves to `{}`, the flag alone CLOBBERS a caller that populated `_GLOBAL_CONFIG` by hand (every test that stuffs a dict in). ⚠⚠ **One new side effect had to be REFUSED, not accepted**: `load_config()` auto-creates a default `config.jsonc`, and a READ that writes into the user's storage dir is a worse surprise than the defect being fixed - the lazy path passes `create_missing=False`, and values are unchanged because the file it declines to write IS the template and the template IS `DEFAULTS`. It also never raises (a read that starts failing is worse than a silent default) and leaves the flag unset on failure so the next read retries. ⚠ `conftest.py` now resets config state BEFORE each test as well as after: teardown alone left `_GLOBAL_CONFIG` `{}` until the first test's teardown, and a lazy load out of that state pulls the DEVELOPER's real `~/.code-index/config.jsonc` into whichever test reads config first - the #411 family. Tests: `test_config_lazy_load.py` (18; **13 fail pre-fix**, 5 controls pass BOTH sides), reproductions in SUBPROCESSES because a fresh interpreter is the only honest way to reach "nothing has been loaded yet".
- **Prior (1.108.257):** **A response limit that is actually a response limit** (#425). Nothing bounded a single MCP tool response. What bounded it in practice was `max_file_size`, an **INDEXING** limit, in a different subsystem, doing the job **by coincidence**, with no test pinning the relationship. ⚠⚠ **Raising `max_file_size` to index a large generated file silently raised the largest reply the server could emit** - that key was made settable in .193 for indexing COVERAGE and never claimed to govern transport payload size. New `response_max_bytes` (`JCODEMUNCH_RESPONSE_MAX_BYTES`), default **1 MiB**, resolver `get_max_response_bytes()` in the same shape as its 3 siblings; `0` is an explicit opt-out and anything else invalid falls back to the DEFAULT, because a typo must never mean no ceiling. ⚠⚠ **Enforced in a wrapper AROUND the dispatcher, not inside it**: `call_tool` is now the wrapper and `_call_tool_impl` the dispatcher, which has 12+ `return` sites across MUNCH / JSON / in-band-error / front-door paths, so a check at any one of them is a check the next branch will not have (same reasoning as `evidence/producers.mint()`). The front door's `order`/`route` re-dispatch passes through it too. ⚠ **It REFUSES rather than truncating** - a shortened body is indistinguishable from a complete one, so silent truncation is the one outcome worse than an error; the error names size, limit and the key that moves it. An already-failing result passes through untouched (capping an error swaps a specific diagnosis for a generic one) and a raising cap is swallowed (a cap that fails CLOSED is worse than no cap). ⚠⚠ **The report's 'not verified' note was a HARNESS artifact, so this landed LESS severe than filed**: `config.get()` reads `_GLOBAL_CONFIG`, empty until `load_config()` runs, so EVERY env-mapped key resolves to its default in an unloaded process (`max_folder_files` / `max_index_files` / `staleness_days` behave identically - it is not about `max_file_size`). `index` / `index-file` / `serve` all dispatch AFTER the shared `load_config()`, so **no production path resolves that limit unloaded**. ⚠ The underlying sharp edge (a silent default instead of a not-loaded signal) has already produced THREE one-subcommand-at-a-time patches in `server.py`; it needs its own issue, not a fourth. Tests: `test_response_cap.py` (29) with an END-TO-END refusal through `call_tool` plus a control proving the refusal is the cap and not a broken tool. ⚠⚠ **The rename made `test_dispatch_schema_parity.py` pass VACUOUSLY** - it walked `call_tool` by NAME for `name == "<tool>"` branches, so after the rename it parsed the thin wrapper, found ZERO branches, and its parity assertion passed over an empty set. Nothing in that assertion failed; the **guard-the-guard** (`test_dispatch_chain_is_parseable_and_nonempty`, asserts >50 branches) is the only thing that caught it. **Any test that locates its subject by function NAME is one rename from inert** - pair it with a non-vacuity floor.
- **Older releases (1.108.256 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **Tests:** 7290 passed, 7 skipped, **0 failed** (1.108.259, full local run POST-bump, 16m08s). ⚠ The delta from .258's 7258 is EXACTLY the one new file: 32 from `test_refresh_campaign.py` (#395); nothing else moved. ⚠⚠ **A config change is the one edit whose blast radius is the whole suite** - 128 test files touch `_GLOBAL_CONFIG` directly, so a "small" resolver change is never a small run. ⚠ **The "KNOWN 12 local-ONNX `test_semantic_search` env failures" are GONE** — .207's autouse `no_local_onnx` fixture fixed them, so a local run is now fully green and **any** red is a real signal. Do not carry that 12-failure allowance forward; it papered over a real failure once already (.197 had one hiding inside it). ⚠ **Still do not eyeball the COUNT** — diff the FAILED names against the same tree with your changes stashed; for .199 and .205 that diff was empty, and for .209 the failure set was empty outright, which is the one case that needs no baseline. ⚠ **Stashing is the wrong tool when the change is already committed and pushed** — for .205 the comparison ran in a throwaway `git worktree add --detach <pre-release-sha>`, which also survives a concurrent writer in the main tree.
- **Python:** >=3.10
