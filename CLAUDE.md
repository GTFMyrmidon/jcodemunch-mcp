...
## Current State
- **Version:** 1.108.237 — **a YAML symbol's byte extent describes real source, not a reconstruction**. `_append_virtual_symbol` took `byte_offset` from the FILE and `byte_length` from `len(signature)`, and the signature is REBUILT from parsed data, never quoted from source — two coordinate systems, so their product sliced arbitrary bytes. ⚠⚠ **Held for 237 of 237 virtual symbols.** The `uses` key in `health-radar-comment.yml` (reconstructed signature 74 bytes) returned 74 bytes spanning two entirely DIFFERENT sibling keys; `name: Health Radar Comment` is 27 source bytes reconstructing to 28 because the rebuild quotes the scalar. Extent now describes the symbol's actual line, and `content_hash` covers the same bytes the extent names — which is what lets `verify` succeed AT ALL. ⚠⚠ **`content_verified` was False for YAML while the verdict still read `ok`**; after the fix `byte_length == len(signature)` fell 100% -> 1.6% (residue = lines whose real length coincidentally matches) and `content_verified` is true for 1415/1422. ⚠ **An unlocatable symbol gets a ZERO extent, not a plausible slice** — refusing beats confidently returning somebody else's text; 7 symbols here take that path, all with a line past EOF. ⚠⚠ **This ISOLATES a second YAML defect rather than fixing it: `line` is still mislocated for ~27% of YAML symbols** (`_find_line` scans forward from a monotonic cursor, so a nested key finds a LATER sibling's line). Affected symbols now return a REAL line instead of fabricated bytes — strict improvement, possibly the wrong line. Previously the two compounded indistinguishably; any residual wrongness now has exactly ONE cause. ⚠ A third, unfixed: `on:` is coerced to a YAML 1.1 boolean, producing a symbol literally named `True`. ⚠ No `INDEX_VERSION` bump; existing indexes keep old extents until re-parsed. Tests: `test_yaml_byte_extent.py` (8), which assert the extent is a real, whole, self-consistent line and say NOTHING about which line, so neither defect's regression can hide inside the other's test.
- **Prior (1.108.236):** **a TOML table no longer claims the next table's header line**. `end_line` for a `table` ran one row past the table's own text: against `fastapi/fastapi` at `a64dfbbd`, `[build-system]` (lines 1-3, blank at 4) reported `end_line=5`, the line holding `[project]`, and `[tool.mypy]` reported 206, the line holding `[[tool.mypy.overrides]]`. ⚠⚠ **The cause is NOT TOML-specific and is worth carrying: a tree-sitter node whose `end_point` sits at COLUMN 0 stops at a line BOUNDARY and holds no text on that row**, so the `end_point[0] + 1` idiom overshoots by one. It surfaces here only because tree-sitter-toml runs a `table` node to the start of the FOLLOWING table, which puts the boundary exactly on another symbol's header. ⚠ **`byte_length` was correct throughout** — the two fields disagreed and only the line number was wrong, which is why nothing reading the body ever noticed; a test now pins their agreement so drift in EITHER fails instead of being absorbed by the other. ⚠⚠ **Deliberately scoped to the three TOML sites out of 73 sharing the idiom, and the scoping was MEASURED not assumed**: sampled across two corpora, python/go/typescript/javascript/json/css showed ZERO disagreement because their symbol nodes end at the last token, so sweeping all 73 would move line numbers for languages with no demonstrated defect. ⚠⚠ **A separate YAML discrepancy is NOT addressed here, and it is a DIFFERENT FIELD** — 15 of 39 sampled YAML symbols have `end_line` and `byte_length` disagreeing, and spot-checking says the culprit is the opposite of TOML's: **`end_line` is right and `byte_length` REACHES PAST the symbol** (`health-radar-comment.yml::name` is 27 bytes of text recorded as 28; a sampled step key recorded 74 bytes covering two sibling keys). ⚠ **The first characterisation written here was BACKWARDS** — it named `end_line` as undershooting, on a measurement that only established that the two fields disagree, never which one was wrong. Establish the direction before naming a culprit. Blast radius is worse than TOML's (an over-long extent puts content in a body that is not the symbol's), so it gets its own investigation, NOT a bundle into a line-number fix. ⚠ **No `INDEX_VERSION` bump: existing indexes keep the old value until the file is re-parsed**; re-indexing every user's world for a TOML line number is not a trade worth making. Reachable from bounded-mode `source_end_line` clamping, `get_file_outline` spans, and diff-to-symbol mapping. ⚠ **Verified at the CALLER's entry point, not only the parser** ([[feedback_verify_at_the_users_entry_point]]): 119 TOML symbols walked through `get_symbol_source` on a freshly built index, zero line-span disagreements, against 11-of-40 before. Tests proven NON-VACUOUS (all four fixture tables differ under the old expression). Same family as #378's left-recursive `dotted_key` bug, where the shallow cases looked correct. Tests: `test_toml_end_line.py` (7).
- **Prior (1.108.235):** **a cache truncated inside its last line is no longer attested as matching** (#412, @rknighton; a follow-up to his own #401, whose patch introduced the prefix match this closes). `_slice_matches` sliced to `len(cached_slice)` — a PREFIX match — and rejected only on a differing newline COUNT. Cutting bytes from INSIDE the final line preserves that count, so `return 4` against a committed `return 42` reported **`git_sha_match`**. Both verdict paths share the helper (`get_symbol.py:263` and `:276`), so one defect produced a false match on the recorded commit AND a false `git_sha_uncommitted` against the working tree. ⚠⚠ **The comment above the guard CLAIMED the line-count check ruled truncation out. It did not, and that false reassurance is why this survived a review looking straight at it** — corrected in the same commit. ⚠ **Measured at the CALLER's entry point, not only at the helper, and it CUTS AGAINST the report:** `content_verified` (per-symbol `content_hash` from index time) is computed three lines above the git_sha branch and returns **False** on exactly these caches, so the response was internally CONTRADICTORY, not uniformly confidently wrong. A real mitigation the report could not see, because it exercised `_slice_matches` directly. It does NOT excuse the defect: `git_sha_verification` is the externally-attested signal, its `verify`/`verify_against` ride an evidence receipt's canonical projection as `mode_args`, and a direct caller of the helper has no sibling to contradict it ([[feedback_two_paths_one_decision]]). **The fix keeps the one legitimate shortfall** — trailing text on the last line stays uncompared, because text that TRAILS a symbol is separated from it while a truncation cuts mid-token, so requiring the remainder to begin on whitespace splits the two without knowing the language. ⚠ **Deliberately biased toward REFUSING and the source says so**: `return 42# note` now reports divergence; a false mismatch costs a re-read, a false match attests bytes nobody checked inside a provenance claim. ⚠ **Both guards are load-bearing, verified BOTH ways** — line-count alone admitted #412; the new boundary check alone would admit a cache missing its whole final line (the remainder then starts with `\n`, which is whitespace). Verified end-to-end through `get_symbol_source` against a real git repo with `byte_length` corrupted in SQLite; all three states now agree across both signals. His regression test included verbatim. Tests: `test_v1_108_224.py` (21).
- **Older releases (1.108.234 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **INDEX_VERSION:** 17
