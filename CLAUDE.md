...
## Current State
- **Version:** 1.108.260. **A receipt that confirms your typo** (found re-checking #424; NOT the issue, which stays jjg's call). `JCODEMUNCH_TOOL_SURFACE=countr` **reported itself back verbatim while serving the full 91-tool surface**, because only `"counter"` is ever special-cased - so someone who typo'd and went looking for why their token cost never dropped read a receipt that **CONFIRMED the setting was accepted**. ⚠⚠ **Fifth occurrence of [[feedback_diagnostic_must_resolve_like_the_runtime]]** (after .250 and .255): a diagnostic must resolve its value the way the RUNTIME does, or it is worse than no diagnostic because it is believed. `_effective_surface()` now always returns a member of `VALID_TOOL_SURFACES`; unrecognized values resolve to `full`, which is what they have always **DONE** - only the REPORTED value moves. ⚠ **Silently normalising is NOT enough on its own** - it hides the typo in the other direction, so the receipt names the rejected value (`surface_requested` / `surface_unrecognized` / `surface_note`, omitted entirely when clean) and a ONE-TIME warning names the value, the valid set and the knob. Config validated on the same path as env (the key shares the hazard). ⚠ **Evidence caveat, stated rather than dressed up**: `test_tool_surface_clamp.py` (25) CANNOT run against the pre-fix module (the symbols did not exist), so it is not a "25 fail pre-fix" file; the defect was captured EMPIRICALLY before the change. Non-vacuity floor asserts `counter` and `full` are actually distinguishable. ⚠ **Decision-neutral on #424 by construction**: the telemetry payload is `{delta,total,anon_id}` and carries NO surface dimension - verified, and now pinned by a test so it stays a DECISION instead of drifting in as a refactor's side effect.
