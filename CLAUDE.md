...
- **Prior (1.108.256):** **The registry entry advertised a claim the README had already retired.** `server.json` is what the MCP registry publishes and what mcp.so / MCPFind / mcprepository / PulseMCP DISPLAY; its `description` still read `95%+ token savings`. ⚠⚠ That number predates **.233**, where the re-measured benchmark moved the headline AGAINST us on purpose (237.3x to 27.9x, 99.6% to 96.4%) and the README was corrected to `86-99% (96% average)` because per-query results span 86% to 99% and the aggregate alone overstates the low end BY OMISSION. **The README got the fix and its sibling did not**, so the retired claim stayed live on every aggregator mirroring the registry: [[feedback_guard_every_path_that_shares_the_hazard]] in a marketing costume. ⚠ **A correction to a shipped claim rides a RELEASE, not a bare commit** (jjg, 2026-08-07) - the point of the registry version is that a stale entry is detectable at a glance, and that only holds if server.json, PyPI, the GH release and the plugin manifest all name ONE version. ⚠ The `description` field caps at **100 chars and validates BEFORE auth**, so an overlong string fails after the device-flow login, not before it; the new text is 98. No behaviour change, no test delta.
- **Prior (1.108.255):** **Hook output written to channels the model never sees** (#420, @georgebashi). For a Claude Code hook that exits 0, `hookSpecificOutput.additionalContext` is the ONLY channel that reaches the model; stderr and top-level `systemMessage` both surface to the USER. (Exit 2 does feed stderr to the model, but it also BLOCKS the call, which is not what an advisory nudge wants.) The Read nudge, the Grep nudge and the SubagentStart repo briefing were all on a user-facing channel. ⚠⚠ **The hooks fired, gated correctly, computed the right text, and none of it landed**. Every observable signal said it worked. The Read nudge had been inert since v1.22.5, which correctly replaced a hard `Read` deny with an exit-0 stderr warning to unbreak Read-before-Edit (#241) and, in moving off the deny channel, moved off the only channel that reached the model. ⚠⚠ **Nine tests in `test_hooks.py` asserted `"search_text" in err` and passed BECAUSE the message went nowhere.** New **`hook-sessionstart`** (matcher `compact|resume|fork`) closes the other half: PreCompact has no model channel, so the snapshot was computed at the exact moment that state was about to be lost and then DISCARDED; SessionStart re-injects it afterwards, both sharing one builder so they cannot describe the same session differently. ⚠ Silent on `startup|clear` (an unrelated session's journal presents stale files as current focus), and it deliberately does NOT inject the no-journal fallback, which is written for a human and reads to a model like a finding. ⚠ **TaskCompleted stays on `systemMessage` BY DESIGN**: that event has no non-blocking model channel and its only alternative, exit 2, would refuse task completion over advisory findings. ⚠ `_note_transcript_root` (#421) is registered from SessionStart BEFORE the source gate. The root is a property of the session, not of whether we inject, and this is a whole session earlier than the first Read or Edit. **Second defect, found reviewing the first**: `config --check` kept its OWN hand-written hook list and tested presence with the substring `jcodemunch-mcp <sub>`. ⚠⚠ `_hook_invocation()` writes an ABSOLUTE path whenever `shutil.which` resolves, so a real install reads `C:/.../jcodemunch-mcp.EXE hook-pretooluse` and the substring is not in it: **the check reported EVERY hook not installed on a correctly-installed machine**. The list had already omitted `hook-sessionstart` and displayed `PreToolUse(Read)` against an installer writing `Read|Grep`. Both now DERIVE from `_enforcement_hooks()` and match via `_extract_jcm_subcommand`, the helper written for those path shapes. Tests: `test_hook_output_channels.py` (pins CHANNELS, not message content) + `test_config_check_hooks.py` (9; **8 fail pre-fix**, 1 control passes BOTH sides, asserted against the CLI's real stdout because the defect was invisible from every layer beneath it).
- **Older releases (1.108.254 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **Tests:** 7240 passed, 7 skipped, **0 failed** (1.108.257, full local run POST-bump, 16m49s; .256 is a description string, no test delta). ⚠ The delta from .254's 7170 is EXACTLY the two new files: 32 from `test_hook_output_channels.py` (#420) and 9 from `test_config_check_hooks.py`; nothing else moved. ⚠ **The "KNOWN 12 local-ONNX `test_semantic_search` env failures" are GONE** — .207's autouse `no_local_onnx` fixture fixed them, so a local run is now fully green and **any** red is a real signal. Do not carry that 12-failure allowance forward; it papered over a real failure once already (.197 had one hiding inside it). ⚠ **Still do not eyeball the COUNT** — diff the FAILED names against the same tree with your changes stashed; for .199 and .205 that diff was empty, and for .209 the failure set was empty outright, which is the one case that needs no baseline. ⚠ **Stashing is the wrong tool when the change is already committed and pushed** — for .205 the comparison ran in a throwaway `git worktree add --detach <pre-release-sha>`, which also survives a concurrent writer in the main tree.
- **Python:** >=3.10
