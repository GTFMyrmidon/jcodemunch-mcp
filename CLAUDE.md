...
## Current State
- **Version:** 1.108.238 — **a YAML symbol is located at its own key's line, or refuses**. Line numbers came from a forward text scan with THREE compounding defects, measured on `health-radar-comment.yml` where a five-key step block resolved to 31/32/33/34/35 and only the last was right: (1) **substring matching** let `id:` answer for `run-id:`; (2) **a cursor starting past the item's own key** — a list item is identified by its `name:` line and the walk began on the FOLLOWING line, so the item's own `name` bound to a nested `name:` further down; (3) **a silent fabricated fallback** returning `after + 1`, i.e. the next line dressed as a match (2 of the 5 keys), and because each wrong answer advanced the cursor the errors CASCADED through every sibling after. True lines now come from YAML node marks (`yaml.compose`), anchored key search as fallback; an unlocatable key reports NO line and a ZERO extent. ⚠⚠ **Heuristic repair was tried and ABANDONED ON EVIDENCE**: anchoring + refusing took ~73% -> 96.5%, then advancing a parent cursor past nested blocks made it **WORSE (89.6%)** — a text cursor has no notion of where a block ENDS, so every heuristic trades one mislocation class for another. ⚠⚠ **The first node-mark version SILENTLY DID NOT RUN** — it said `yaml.compose` but this module imports yaml only as a local `_yaml`, so `NameError` -> broad `except Exception` -> empty map -> the fallback kept answering while the measurement still read healthy. Handlers are now narrow (`ImportError`/`YAMLError`) and a test asserts the map is non-empty. ⚠ **The node-mark oracle check (462/462) is CIRCULAR** — it shares a mechanism with the fix; the load-bearing number is the INDEPENDENT one: asserting the located line actually BEGINS with its key, **0 of 4773 on a wrong line** (was ~27%), 9 honest refusals. ⚠ **`on:` -> `True` YAML 1.1 coercion is now VISIBLE rather than hidden and is NOT fixed here** — it refuses instead of fabricating. ⚠ `_find_line` is RETAINED for Ansible task/role paths (free text, no key to anchor); nothing measured there, nothing changed there. ⚠ No `INDEX_VERSION` bump. Tests: `test_yaml_line_location.py` (13).
...
- **Older releases (1.108.235 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **INDEX_VERSION:** 17
