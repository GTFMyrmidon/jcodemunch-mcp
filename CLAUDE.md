...
## Current State
- **Version:** 1.108.231 — **only signals that discriminate get a vote** (#408 CLOSED, the arithmetic half). `confidence` was `len(signals)/3.0`, an unweighted vote in which a signal firing on EVERY symbol still contributed a full third. A signal whose measured fire rate is degenerate now gets no vote. ⚠ **The denominator STAYS 3, deliberately** — dividing by the informative count would scale a lone survivor back up to 1.0, i.e. MORE confident than the three-signal case it replaced. Holding it makes the **ceiling** fall, so a repo where nothing discriminates returns nothing **through the caller's existing `min_confidence`**, not through a second invisible suppression rule. New `max_achievable_confidence` + `signal_warning` so an empty result reads as a finding, not a malfunction. ⚠ **`_DEGENERACY_CUTOFF = 0.90` was CHOSEN BY SWEEP, not taste: it is the LARGEST cutoff at which no repo reports >90% of its functions dead.** 0.95 leaves httpx at 93.1% (the exact failure mode); 0.80 over-suppresses (median 0.0%, 17/31 empty). Corpus mean 57.3% -> 27.4%. ⚠ **NOT a blanket suppression** — where all three signals discriminate, answers are UNCHANGED (pylint 74.6%, flask 62.8%, matplotlib 56.8%, astropy 55.4%, next 16.1%). gin/seaborn/httpx 100/100/93.1% -> 0%; jcm 27.2% -> 5.5%. ⚠⚠ **WHAT IT DOES NOT DO, MEASURED: per-item precision did NOT improve and on jcm it got WORSE (3.3% -> 1.7%).** It strips ~590 false positives from jcm but takes most of the true positives with them; recall falls at least as fast as volume. What it reliably fixes is "accuses everything", a different and worse problem than a low hit rate. ⚠ The `check_references` proxy is name-based so it OVER-reports references — those precision figures are a LOWER BOUND, and a different methodology from the 69.6%/64.1% F1, which this does NOT falsify. ⚠ **NEW PARAM `degeneracy_cutoff`** (optional, default 0.90) — the cutoff was chosen from 31 repos that may not resemble yours, so the choice is reversible PER-CALLER; pass `1.0` for near pre-.231 volume. **Only `0.5 < cutoff <= 1.0` is accepted and out-of-range is REFUSED, not clamped** (below 0.5 the band inverts, every signal drops out, and the tool returns nothing for every repo on earth — a clamp would answer a question the caller did not ask and look like it worked). ⚠ **Stripped from the compact schema via `_COMPACT_STRIP_PARAMS`, still honoured** — `get_dead_code_v2` is CORE-tier and `core_compact` sits at **3996/4000**, so a new property does not fit at any description length; same measured reason as `receipt`/`verify_against`. Also updated: `test_v1_108_230.py` (5 -> unchanged count, invariance test superseded in place). ⚠ **Two fixes fell out of the tests: (1) `file_pattern` was DEFINING the population the signals were measured against** — filtering before the rates were computed drove every rate to 1.0 when scoped to one file, so the tool returned nothing for a question it could answer; it is now applied to the OUTPUT only. **(2) A repo where every function looks identical now returns NOTHING** (all rates 1.0, nothing discriminates, honest answer is "cannot tell") — intended, and a sharp edge: three pre-existing pagination tests used exactly that shape as a fixture. `_call_graph_only_dead_code` is UNCHANGED (single-signal, `confidence` fixed at 0.5, self-labelled `mode: call_graph_only`; no vote to reweight). Tests: `test_v1_108_231.py` (16); .230's invariance test is SUPERSEDED in place, not deleted.
- **Prior (1.108.230):** **the dead-code signals report what they measured** (#408, INSTRUMENT ONLY). `_meta.signal_diagnostics` reports per call: `fire_rate` (fraction of ANALYSED symbols each signal fired on — counted over analysed not returned, so the reading does NOT move with `min_confidence`), `cofire_rate` (the testable form of the docstring's "three independent signals"), `uninformative` (rate 0.0 or 1.0), `entry_points_detected` + `degraded`. ⚠ **No verdict and no confidence value changed, and a test asserts it** — `confidence` is still `len(signals)/3.0` and jcm's flagged set is byte-identical to .229. The weighting fix ships separately so it can be read against a measured before. ⚠⚠ **WHAT IT MEASURED, 31 repos (py/go/ts/js/rust): the tool flags a mean 57.3% of all analysed functions as dead.** gin and seaborn return **100.0%**; jdoc 96.8%, langchain 93.1%, authlib 85.2%, django 67.1%. A detector that accuses everything is silent. ⚠⚠ **TWO OF OUR OWN #408 CLAIMS DIED ON CONTACT.** (1) **The `framework_warning` TRIGGER is wrong, not just its consequence**: the issue blames `entry_point_count == 0` (3/31), but `unreachable_file` fires on 100% of symbols in **6/31** — langchain has **7 entry points detected** and is still saturated. `entry_points_detected > 0` is NOT evidence the signal discriminates, so `uninformative` derives from the MEASURED RATE, never the entry-point count. (2) **The correlation claim is WITHDRAWN**: measured mean cofire is **0.312**, not "strongly correlated", and the two rates usually move in OPPOSITE directions. That was reasoning from the shape of the signals instead of from data. ⚠ **The real driver is `not_barrel_exported`, which nobody suspected** — rate >0.80 on ~2/3 of the corpus, >0.90 on a dozen (gin 1.000, jdata 0.968, fastapi 0.954, jcm 0.943). Most symbols start at 0.33 from it alone, so **the default `min_confidence=0.5` reduces to "any ONE signal other than `not_barrel_exported`"**. The three-signal vote is a one-and-a-half-signal vote and the free third fires almost everywhere. Tests: `test_v1_108_230.py` (5).
- **Prior (1.108.229):** **the dead-code check stops losing module-level callers** (#409, in-house). `get_dead_code_v2` reported **three shipped MCP tools** (`get_pr_risk_profile`, `get_symbol_provenance`, `index_repo`), every `main()` and all five CLI hook entry points as dead — 50 live symbols on jcm's own tree. `CodeIndex.get_callers_by_name` is built from **per-symbol** `call_references`, so module-level code belongs to no symbol and never enters it; that is where CLI wiring, route tables, framework registration and `__main__` blocks live. ⚠⚠ **What made it invisible: the path was selected by `if callers_by_name:`, a REPO-WIDE property.** A tree where no symbol called another produced an empty (falsy) dict and fell through to the text heuristic, **which gets these right**; adding one function that calls a sibling ANYWHERE flipped the whole repo onto the AST path, which structurally cannot see an import-time call. A symbol's verdict changed on an unrelated edge between two OTHER symbols — and since every real repo has such an edge, **the fallback that answered correctly was unreachable in production**. Both paths now run: AST first (keeps its precision), then a sweep over only what it left unresolved, against each file's **module-level residue** (file text minus every symbol's line span) — exactly the fast path's blind spot. ⚠ **Cost is MEASURED, 2.3x**: 922 ms → 2,110 ms on jcm (9,697 functions / 743 files), 816 flagged → 766, **zero newly flagged** (strictly monotone in the safe direction). ⚠ The first implementation scanned residue TEXT per symbol and measured **11,344 ms — 12.3x, worse than the whole tool**; tokenising each residue once into a name set and testing membership (the same shape as `called_names_by_file` directly above it) recovered it **without changing one verdict**. Files are read only for symbols the AST pass did not resolve. ⚠ **Disclosed residual: it matches text, not calls**, so a bare module-level mention (an `__all__` entry, a decorator above a neighbour) counts as a caller — the trade the text fallback already made, and the safe direction for a signal feeding deletion decisions. ⚠⚠ **THE FIXTURE LESSON: a test needs BOTH an intra-symbol call AND a module-level call in the same tree.** Either alone passes on the broken code, so the obvious two-file fixture takes the fallback and proves nothing — which is why this survived. Stashing the fix fails exactly 2 of 6; the other 4 are guards. **#408 (confidence scores a signal the same response declares invalid) is the sibling finding and is deliberately NOT in this release** — it changes confidence arithmetic, and the instrument ships separately from the fix or the before/after is unreadable. Tests: `test_v1_108_229.py` (6).
- **Older releases (1.108.228 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **INDEX_VERSION:** 17
- **Tests:** 6709 passed, 7 skipped, **0 failed** (1.108.231, full local run, 13m52s). ⚠ **The "KNOWN 12 local-ONNX `test_semantic_search` env failures" are GONE** — .207's autouse `no_local_onnx` fixture fixed them, so a local run is now fully green and **any** red is a real signal. Do not carry that 12-failure allowance forward; it papered over a real failure once already (.197 had one hiding inside it). ⚠ **Still do not eyeball the COUNT** — diff the FAILED names against the same tree with your changes stashed; for .199 and .205 that diff was empty, and for .209 the failure set was empty outright, which is the one case that needs no baseline. ⚠ **Stashing is the wrong tool when the change is already committed and pushed** — for .205 the comparison ran in a throwaway `git worktree add --detach <pre-release-sha>`, which also survives a concurrent writer in the main tree.
- **Python:** >=3.10
