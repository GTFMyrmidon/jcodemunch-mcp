...
- **Prior (1.108.226):** **the dead-code check stops calling live code dead** (@rknighton, #406 + #407). **#406(1): `check_references` discarded the defining FILE when only the DEFINITION should be excluded** — `defining_files` was a set of PATHS and the loop `continue`d past them, so EVERY SAME-FILE CALL SITE was lost and a helper defined-and-used in one module read `is_referenced: false`. Now excludes the definition's LINE SPAN, as a LIST of spans per file (102 duplicate `(file,name)` pairs measured on a real 1837-symbol index — one span per file would leave the others counting their own signatures). Self-recursion stays excluded (correct for the dead-code question). ⚠⚠ **A regression THIS FIX would have introduced, caught by an EXISTING test not a new one: a symbol with no usable `line` has nothing to exclude, so counting its file reports the DEFINITION as a reference to ITSELF** — worst on the indexes with the LEAST metadata. `unspanned_files` degrades to the pre-.226 whole-file skip. **Unknown is a third bucket, not a default.** **#406(2): `run_taskcomplete` read `total_references`, a key `check_references` has NEVER returned** in either mode — so `== 0` was ALWAYS true and every inspected symbol was labelled unreferenced in the agent-facing message (the #338 class; `7d0e996` swept the other two callers and missed `cli/hooks.py`). A test now fails on any READ of that key anywhere in `src/`. ⚠⚠ **NEITHER ISSUE REPORTED THE WORST ONE: `check_delete_safe` said `safe_to_delete` for a symbol called ONE LINE BELOW IT.** Both safety preflights skipped `ref_file == target_file` — #406's defect one layer up — and **the skip was UNREACHABLE before .226**, so nothing could show it was wrong. Now `internal_uses_blocking` + an `internal_reference` blocker (machinery already existed); a genuinely unused symbol still clears. **Verdict change in a safety tool, in the SAFE direction.** **#407: `httpx.AsyncClient()` defaults `follow_redirects=False`** — GitHub 301s a RENAMED/TRANSFERRED repo, which doesn't match the `(403,429)` retry branch, so **every renamed repo was unindexable**; both sites now pass it. ⚠ Token-safe: httpx drops `Authorization` cross-origin. 7 of the new tests fail on .225; all 18 pass. Tests: `test_v1_108_226.py` (18).
- **Prior (1.108.225):** **freshness stops contradicting itself, twice** (@rknighton, #405 + #404, a matched pair). **#405: `FreshnessProbe.classify()` returned `stale_index` the instant the index SHA differed from live HEAD — TEN LINES ABOVE the per-file mtime comparison proving the file byte-identical to what was indexed.** One commit touching one file marked EVERY file stale. ⚠ **Not the safe-direction caution it looks like: `unknown` costs nothing and invites a CHECK; `stale_index` is a POSITIVE CLAIM that invites a REINDEX**, asserted against a measurement already taken and not read — so on a repo under active commit the signal carried NO information exactly where it should discriminate (the cost side of #395). Per-file evidence now runs first; the repo SHA still decides the LABEL for a file that DID move (so a committed change reads `stale_index`, not `edited_uncommitted`). ⚠⚠ **CONTRACT CHANGE: `repo_is_stale: true` beside rows reading `fresh` is now LEGITIMATE** — the repo moved, those files didn't. Anyone expecting "stale repo ⇒ stale rows" is reading the old contract. **#404: a revalidated cache hit still reported `fresh`** — the verdict disclosed `revalidated.stale_cache: true` while the rows kept the `_freshness` stamped at cache-FILL time, so ONE payload asserted both, and the unsafe direction (vouching for stale content). ⚠ NOT a reversal of .178's cached-positive policy — the disclosure reached `verdict` and STOPPED. ⚠ **`_result_cache_get` now copies the ROWS and that half is load-bearing**: it copied the top level and `_meta` but not the row dicts, so in-place annotation would write through to the stored entry and replay to every later hit (the `evidence_ref` leak of #377 item 3, one level down). ⚠ **Third correction NOBODY asked for, disclosed: an entry with no `file` on a stale repo went `stale_index`, now `unknown`** — a positive claim about a path we don't have. ⚠ Two guards are OURS not the reports': **`file_mtimes` can be SPARSE** (a file absent from it must not inherit a sibling's exemption) and the .209 invariant re-asserted on the branch the reorder MOVED. ⚠ Residual disclosed, not fixed: **mtime is a proxy for content** — checkout-and-back advances mtime without changing bytes (safe direction; the exact answer is a `git diff` subprocess per call). 8 of 14 new tests fail on .224; the 6 passing on both are the guards. Tests: `test_v1_108_225.py` (14).
- **Older releases (1.108.224 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **INDEX_VERSION:** 17
