...
## Current State
- **Version:** 1.108.195 — **The size-cap exemption compared RAW WINDOWS PATHS.** ⚠ **Windows ONLY; POSIX unaffected** (`os.path.normcase` is the IDENTITY function there, so a raw compare cannot fail on Linux/macOS). `_should_index_file` tested `resolved_str not in cfg.forced_paths` while the gitignore check FOUR LINES ABOVE already used `resolved_norm = os.path.normcase(...)` for exactly this reason. On Windows the same file resolves to strings differing by drive-letter case or 8.3 short form, so the membership test missed and **a package.json entry point over the cap was dropped as `too_large` despite being named in `forced_paths`** (#25 silently voided). Both sides now normalise. ⚠⚠ **FOUND BY CI AFTER .194 SHIPPED ON A GREEN *LOCAL* WINDOWS SUITE — a green local Windows run is NOT a green Windows run.** Two CI runs failed on two DIFFERENT Pythons (3.13 then 3.11), which is what established Windows-wide rather than version-specific; `fail-fast` had been cancelling the siblings. ⚠ **The defect is CASING-DEPENDENT and therefore INTERMITTENT: a green Windows run on a build without this fix is LUCK, NOT EVIDENCE.** ⚠ `test_v1_80_9_lodash_class::test_resolves_main_field` updated from `str(big.resolve()) in forced` to the normcase form — same claim, byte-identical on POSIX, and the ONLY test in that class spelling a full path. Suite 6214 passed / 7 skipped.
- **Prior (1.108.194):** **The escape hatch reached ONE walk out of three.** ⚠⚠ **1.108.193 DID NOT FIX THE REPORTED PROBLEM.** It added `max_file_size` + `JCODEMUNCH_MAX_FILE_SIZE` + a resolver and wired `get_max_file_size()` into **exactly one call site — the watcher fast path**. `index_folder` calls `discover_local_files` WITHOUT `max_size=`, so the primary walk kept the hardcoded default: cap raised to 2,000,000 → resolver returns 2000000, discovery still reports `too_large:1` and finds NOTHING. **"The size cap cannot be moved" was still true after the release that claimed to move it — the same defect shape .193 was written about.** Fix resolves ON ENTRY in `discover_local_files` + `resolve_explicit_paths`, beside the existing `max_files = get_max_folder_files(max_files)` line, so every caller is correct by default. ⚠ **Second bug: the #25 package.json size-cap exemption never reached the incremental path** (`forced_paths=set()` hardcoded), so an oversize entry point was indexed by a full walk and dropped as `too_large` by an incremental one — it APPEARED THEN SILENTLY VANISHED on the next edit. Scan stays LAZY (it is a tree walk; only a change set containing an oversize file pays) or the fix would trade this bug for the #375 stall class. ⚠ **Third: `config` never reported `max_file_size`** — listed beside its two siblings, its absence read as "not there" rather than "not shown", which is why @dkiaulakis probed the resolver by hand and concluded the env route was dead (it was not; his bare REPL never called `load_config()`). Now renders `2000000 [env]` with provenance. ⚠⚠ **The .193 test asserting the literal source string `max_size=get_max_file_size()` was rewritten to be BEHAVIOURAL — a source grep cannot tell a refactor from a regression, and rewriting it is what EXPOSED the primary-walk bug** (the spy saw zero resolver calls during a normal index). ⚠ **My first parity test was VACUOUS and passed with the bug present** (asserted on `discovery_skip_counts`, which the incremental result does not carry); the discriminator is `changed` (1 vs 0). New `tests/test_v1_108_194.py` (9), 4 fail on revert. **ALL THREE found by following @dkiaulakis's RETRACTION** — he withdrew the original finding after discovering `should_exclude_file` has ZERO production callers (dead code with a full test suite around it, which is what made it look authoritative). Suite 6213 passed / 7 skipped.
...
- **INDEX_VERSION:** 17
- **Tests:** 6214 passed, 7 skipped (1.108.195) + the KNOWN 12 local-ONNX `test_semantic_search` env failures (green in CI on all 4 ubuntu jobs — never read them as a regression)
- **Python:** >=3.10
