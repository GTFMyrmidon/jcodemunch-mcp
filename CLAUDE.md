...
## Current State
- **Version:** 1.108.200 — **A staleness warning that fires on nearly every read protects nothing.** SCIP P3. `scip_meta_and_stale` compared the git HEAD at ingest against the index's current HEAD, which is REPO-GLOBAL: **one commit touching one file SCIP never covered marked EVERY edge stale, on every read, in all five consumers** (`find_references`, `get_blast_radius`, `get_call_hierarchy`, `find_implementations`, `check_edit_safe`/`check_delete_safe`). A moved HEAD is NECESSARY for the evidence to be stale, not SUFFICIENT. ⚠ **Same reasoning that narrowed the 1.108.181 working-tree gate — a refusal appearing on almost every answer is one people learn to click past, at which point it protects nothing.** Ingest now records the content hash of every file its edges touch (both endpoints, derived from symbol-id path prefixes since `scip_edges` has NO file column) and staleness requires a covered file to have changed. ⚠ **Stored as ONE JSON value under a new key in the EXISTING `scip_meta` key-value table — no new table, no new column, so NO `INDEX_VERSION` bump and no observatory cache-key bump.** ⚠ **FAILS TOWARD STALE: a pre-.200 ingest has no map, an unreadable map, an empty map and a failed lookup ALL keep the old head-only answer — absence of provenance is unknown, NEVER clean.** ⚠ **ZERO consumer edits: all five read staleness through the one function, so the refinement reaches them without touching any.** Also: `digest` had NO scip reference at all, so an agent could not tell compiler-verified references existed — the one fact that changes how far to trust `find_references`/`check_delete_safe`; the briefing now carries edge count, indexer, and currency, and is ABSENT on repos with no ingest so those digests stay byte-identical. Docs shipped separately as `SCIP.md` (per-language recipes, CI ordering, limits) — **P2 was already complete and both CLAUDE.md and memory understated it at two consumers when five were wired.** Tests: `tests/test_v1_108_200.py` (9); **1 fails on revert — the one that IS the behaviour change — the other 8 are fails-toward-stale invariants that pass on revert BY DESIGN.**
