...
## Current State
- **Version:** 1.108.212 — **`route()` can reach the three transform actions it could never propose.** `render_diagram` / `plan_refactoring` / `assemble_task_context` consume another tool's OUTPUT rather than querying the index, so the words a user types for them appear nowhere in the catalog text the lexical fallback ranks over, and no curated rule covered them. **Measured route recall for that group was 0%** — capability shipped, route to it did not. Three `_INTENT_RULES` entries + broader task-setup phrasing for `assemble_task_context`: **transform 0% → 100% at rank 1, zero regressions**; overall route@3 44.1 → 49.2, route@1 25.4 → 30.5. ⚠ **Placement is load-bearing, not cosmetic**: the transform block sits LATE so a specific data-fetch intent still wins primary ("visualize the call graph" leads with `get_call_hierarchy`, because `render_diagram` consumes that output and has nothing to draw without it), but ABOVE the generic `\bplan\b` rule that would otherwise claim every refactor-plan request. Both directions pinned in `tests/test_counter.py`; the inverted-pair bug was caught BY writing the guard test, not before it. ⚠ **`menu()` is unchanged and still cannot surface these three** — `_INTENT_RULES` is not in its path (name+description lexical only), so browsing still misses them. Stated, not papered over. **New `benchmarks/route_recall/`**: measures `menu`/`route` recall@k separately, per-group, rule-vs-fallback, and refuses to report when a corpus target is absent from the live catalog. ⚠⚠ **Every query carries a measured LEAKAGE score** (how much of the target's own vocabulary the query hands back) — it caught a real 24.5-point inflation on the committed corpus, so quote the CLEAN subset, never the aggregate. ⚠ Single-shot: measures the retriever, NOT the agent loop around it; never read it as a failure rate. **Counter recall gates the `counter` default — see `MEMORY.md`.**
...
- **Older releases (1.108.209 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **INDEX_VERSION:** 17
