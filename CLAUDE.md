...
## Current State
- **Version:** 1.108.221 — **what this install could establish, not just what it SAW.** `evidence/capability.py`, the `munch.corpus-capability/v1` certificate, BOUND into `receipts.coverage_fingerprint`. **coverage = what this SCAN did not see; capability = what this INSTALLATION was ABLE to see.** ⚠⚠ **The live number that made it a real artifact and not a rename: coverage says `complete: true` on jcm's own index while 59 files are skipped as `wrong_extension`** — a reason that conflates "not source code" (correct scope decision) with "this install cannot parse that" (a CAPABILITY gap in a scope decision's clothes), and the walk cannot tell which. #382 proved the mechanism: pack 1.x stopped bundling grammars, 3 registered languages absent, nim's grammar swapped to an upstream we read as empty. **Two installs on different packs have different capability while reporting IDENTICAL coverage.** ⚠ Does NOT make coverage wrong — `WITHHELD_SKIP_REASONS` already refuses `complete` for the config limits (.193); this names the axis the walk never questioned. Bounded BY CONSTRUCTION: counts/versions/digests, no file list (test: <4KB, no source path). ⚠ **Binding it CHANGES minted evidence ids, deliberately** — two different corpora previously shared a fingerprint; ids are session-scoped/in-memory so nothing stored breaks, and `coverage_fingerprint(cov)` with no index is byte-identical to pre-.221 (pinned). ⚠⚠ **Two bugs written AND caught here, both flattering us: `parser_fingerprint` said 157 languages** (LANGUAGE_EXTENSIONS is ext→lang, so `len()` is the EXTENSION count; real = **74**) — a capability document overstating capability 2x; test asserts `languages < extensions`. ⚠ `channel_generations` bound a local `store = EmbeddingStore(path)` OVER the `store` param the SCIP probe needs. ⚠ NOT an MCP tool, test-asserted. Tests: `test_capability_certificate.py` (15).
- **Prior (1.108.220):** **a held-out corpus, and what it said about the last two numbers.** 14 curated intent rules closing .217's defect classes — but the rules are the SMALLER half. ⚠⚠ **`.219` gated the moratorium on `queries.json`, THE SAME CORPUS whose misses were about to be fixed** — fitting rules to a corpus then scoring on it measures MEMORISATION; the report said "held-out" and the word was dropped. New `benchmarks/route_recall/holdout.json` (44 q) **frozen BEFORE the first rule**. On the UNCHANGED router: visible 45.8% vs held-out 20.5% vs held-out-CONTROL 40.0%. ⚠ **Read the control row for like-for-like: the visible corpus flatters comparable queries by ~6 points and its higher leakage (0.133 vs 0.108) is why** — our own leakage metric convicting our own benchmark. **After the rules, held-out by class: `rule_preempted` 8.3→91.7%, `no_lexical_overlap` 0→83.3%, CONTROL 40.0→40.0%**, aggregate 20.5→65.9%; visible 45.8→67.8. ⚠⚠ **Mean name leakage did NOT move (0.057 identical)** — the payoff for fixing with RULES not description edits; stuffing benchmark vocabulary into descriptions fits the measurement, not the product, and raises the metric built to catch it. ⚠⚠ **`test_moratorium_still_in_force` FIRED and the answer was NOT to lift** — the aggregate cleared its bar, but the untargeted control subset is UNCHANGED, so the fixes moved precisely and only what they aimed at and 65.9% is an upper bound on a corpus 24/44 fitted to the fix by the same author. **Gate moved to the CONTROL SUBSET (3rd version; .219=corpus-about-to-be-fixed, .220-first=partly-fitted aggregate, now=untargeted only). NOT a moved goalpost — the previous two both called something held-out that wasn't.** ⚠ Contamination STATED: same author wrote holdout + rules; mirror entries are alternate PHRASINGS not re-picked intents; a third-party corpus is worth soliciting. 3 counterfactual tests broke for the BEST reason (their fixtures were defects this release fixed) → rewritten to CONSTRUCT their gate conditions.
- **Prior (1.108.219):** **the catalog moratorium, with a TEST rather than a promise.** No new top-level actions until stated conditions are met. ⚠⚠ **A moratorium in CONTRIBUTING.md is a HABIT; `tests/test_catalog_moratorium.py` is the POLICY** — `CATALOG_CEILING = 91` pinned, a 92nd action fails with a message naming the exit conditions, proven non-vacuous. Raising it is one constant in one file; the point is not to make an action impossible but to make it DELIBERATE and visible in a diff. **Exit: (1) route@1 >= 60% on the corrected corpus, (2) mean name leakage <= 0.15 at that SAME measurement, (3) generated guidance names only callable actions.** ⚠⚠ **Condition 2's absence would have made the gate ACTIVELY HARMFUL** — "raise route@1 to 60%" is trivially met by paraphrasing tool descriptions into queries, exactly what .218's audit existed to avoid; a recall bar with no leakage bar is an INCENTIVE TO CORRUPT THE CORPUS. ⚠ **Progress is measured from 45.8, NEVER 42.4** (the gap was .218's corpus correction, not routing); a test pins the baseline. ⚠ **2 of the report's 4 conditions DROPPED deliberately**: "clear usage rate per action" needs opt-in-and-off-by-default telemetry, "adjacent tools consolidated" has no failable threshold — a gate nobody can evaluate gets waived on the first argument. **We hold ourselves to it first**: `investigate_deletion_safety` (.214) + `explain_route`/`explain_misses` (.217) are built, tested, and UNEXPOSED. ⚠ **It withholds a SURFACE, never a capability** — a test asserts both stay IMPORTABLE so the policy cannot quietly become deletion. ⚠ **`test_moratorium_still_in_force` FAILS on the day the exit conditions are met** — a policy that expires silently is one nobody notices has expired. Contributor-facing text in `CONTRIBUTING.md` frames what IS still welcome (fixes, languages, params, output fields, docs) since most merged PRs never touched the catalog count. Tests: `test_catalog_moratorium.py` (10).
- **Older releases (1.108.218 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **INDEX_VERSION:** 17
