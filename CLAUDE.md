...
## Current State
- **Version:** 1.108.218 — **the benchmark was wrong about us, in our favour's direction.** Target audit of `benchmarks/route_recall/queries.json`; **NO routing code changed.** ⚠⚠ **route@1 42.4→45.8, route@3 59.3→64.4, menu@3 61.0→67.8 is a CORPUS CORRECTION, not a routing gain — a figure measured after this audit is NOT comparable with one measured before it.** .217's counterfactual showed the corpus violated its OWN stated rule ("'targets' lists every action that would be an ACCEPTABLE answer"): "I do not know this repo..." lost 3x to `get_repo_map`, which IS cold-start orientation and was absent from the target list. All 59 lists re-reviewed → 12 additions, 0 removals. ⚠⚠ **The method IS the result: candidates came from catalog descriptions in ALPHABETICAL order, NEVER from rankings** — "what currently ranks high" as the candidate pool is the leakage mechanism itself, since every accepted correction would then raise the score by construction. **Strongest evidence it held: leakage barely moved (name 0.127→0.133, desc 0.329→0.338)**; paraphrase-matched targets would have moved it WITH the score. ⚠ **The audit deliberately did NOT rescue the misses it could have** — `get_blast_radius` cannot accept an endpoint, so "what breaks if I change the POST orders endpoint" STAYS a miss. ⚠ **12-and-0 is an asymmetry, DISCLOSED not balanced** — 2 over-generous targets (`get_symbol_diff` on a commit-shaped question, `register_edit` which invalidates without re-indexing) were KEPT because the case against each is arguable; manufacturing a removal for symmetry is a worse error. ⚠ The correction SURFACED a miss rather than hiding one: 30→28 explained, `rule_preempted` **6→7**.
...
- **Older releases (1.108.215 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **INDEX_VERSION:** 17
