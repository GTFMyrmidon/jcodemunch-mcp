...
## Current State
- **Version:** 1.108.217 — **every route miss now names the gate that stopped it.** `investigator/retrieval_counterfactual.py`: why was this action not proposed? **All 30 misses in `benchmarks/route_recall/results.json` land on a named gate; NONE unexplained.** `ranked_below_cutoff` 16 (53%, the only bucket ranking work helps) / `no_lexical_overlap` 8 (27%, zero shared tokens, structurally unreachable at ANY weight — this quantifies .213's no-stemming residual) / `rule_preempted` 6 (20%). ⚠⚠ **`rule_preempted` did not exist before this module and it changes what route recall MEANS** — `route` runs the lexical fallback ONLY when `classify_intent` returns nothing, so a rule firing for a plausible-but-WRONG action means the right one is **never scored**, and in the recall figure that is indistinguishable from losing by a point. **No weight tuning can move one of them.** Live: "what breaks if I change the POST orders endpoint" wants `get_endpoint_impact`, `get_blast_radius` claims it; "am I going to break callers if I edit this signature" wants `check_edit_safe`, `get_call_hierarchy` claims it. ⚠ **Fidelity is the whole value** — every gate uses the SAME `counter` functions the live front door uses; a test reconstructs `classify_intent`+`search_catalog` INDEPENDENTLY and asserts agreement on every benchmark query (hits AND misses) plus rank equality; per-token contributions must SUM to `score_action`. ⚠ **The report's own falsifier (">80% of misses reduce to irreducible user ambiguity") came back NEGATIVE — zero do.** ⚠ **NOT an MCP tool, test-asserted** (a 92nd action explaining why the 91st is unreachable is self-refuting). ⚠ **Found, NOT acted on:** "I do not know this repo..." lost 3x to `get_repo_map`, which IS a correct answer the corpus omits — `queries.json` deliberately UNCHANGED, editing the corpus until the number improves is leakage in a different costume. The 6 rule patterns + 8 descriptions are NOT fixed here; shipping an instrument WITH its own recommendations makes it untestable. Driver: `benchmarks/route_recall/explain_misses.py`. Tests: `test_retrieval_counterfactual.py` (13).
- **Prior (1.108.216):** **a narrow read stays narrow, or it promotes.** #398 Arc 2 (@rknighton), on .215's read contract. New `storage/selective.py` + `store.open_selective()`: one read transaction takes repo metadata, ALL file rows, and only the symbol rows the caller named. Reading every file row is deliberate — the `files` table is bounded by FILE count not SYMBOL count, so it is cheap AND it keeps `source_files`/`has_source_file`/`file_languages`/`imports`/coverage EXACT rather than partial. ⚠⚠ **The promotion boundary is `__getattr__`, not an allow-list, and that choice IS the arc** — exact fields are copied onto the instance; everything else falls through and loads the full index once, **including fields that do not exist yet**. Forgetting to update the module makes a request SLOWER, never WRONG; a test proves it with a fake future field monkeypatched onto `CodeIndex`. ⚠ `get_symbol` never promotes (a miss is a targeted single-row read, exact for any id) **but a RAISED fetch promotes rather than returning `None`** — could-not-establish must never be served as could-not-find. ⚠⚠ **NEAR-MISS: the view uses `__slots__` and `_stamp_load_provenance` writes `_db_path`/`_loaded_mtime_ns` inside a bare `except Exception`.** Omitting those two slots sends `subject_state.capture` — which runs on essentially EVERY response — through `__getattr__`, hydrating the whole corpus to answer a question about a file mtime, **while every symbol test still passes**. A silent swallow + a promoting fallback is a perf defect that hides behind green. Wired caller: `get_symbol_source`, byte-identical on hit/batch/miss (the MISS promotes — `did_you_mean` needs the corpus). A non-empty branch returns `None` = *use `load_index`*, never an absence claim. **Measured both sides one interleaved run, cold cache, jcm's own 12,826-symbol index: selective 7.6 ms vs hydrate 159.7 ms = 20.96x.** ⚠ **NOT quotable as the arc's result** — narrowest call, control corpus; 2.24x-2.35x is a mixed-group median and Django/FastAPI are load-bearing and UNMEASURED here. Tests: `test_selective_snapshot.py` (33).
- **Prior (1.108.215):** **one read contract, and a false absence it was hiding.** #398 Arc 1 (@rknighton). New `storage/generation.py` holds both halves: `IndexGeneration`/`describe()` for "which index is this", `connect_readonly()` for "how do I open it". ⚠⚠ **The arc's honest ~1.0x estimate was carrying a live false-absence defect — do NOT price these arcs on the multiplier.** v1.108.185 stated `immutable=1`'s cost as "vectors in an un-checkpointed WAL are not read", i.e. a weaker ranking. **Measured 2026-08-02: an `immutable=1` reader against an un-checkpointed WAL raises `no such table: symbol_embeddings`, and `EmbeddingStore.has_any()` maps that to `False`** — a confident "this repository has no embeddings" about a repository embedded moments earlier. ⚠ **Neither single flag is right: `mode=ro` CREATES the sidecars when absent (the .185 `rebuilding` bug), `immutable=1` cannot see them when present.** Fix reads the WAL when its sidecar is already there and immutably when it is not, so the trade-off DISAPPEARS rather than being balanced. ⚠ **Fourteen call sites shared the hazard in both directions** — ten blind to the WAL (incl. `check_delete_safe`'s runtime probe, an input to a DELETION verdict, and `get_redaction_log`, whose whole job is PII accounting), four creating sidecars (incl. `find_references` and a `token_tracker` loop that did it to EVERY index in `~/.code-index` at once). All fourteen route through one opener; a convention test fails on any hand-rolled read-only URI, exempting `storage/generation.py` BY NAME. Generation half: three surfaces normalised differently (`capture` raw, `producers` `or None`, `verdict` its own baseline), so one index described itself as `""` or `None` depending on who asked. Receipts byte-for-byte identical. Tests: `test_generation_contract.py` (15).
- **Older releases (1.108.214 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **INDEX_VERSION:** 17
