...
- **Prior (1.108.254):** — **Python package-relative imports built NO graph edge at all** (#423, @faxik). `resolve_specifier` read relative imports with **JS path semantics**; Python's form is not a path (`from ..parser.fqn import x` = up one package, then `parser/fqn`), so the join produced the segment `tools/..parser.fqn` and matched nothing. ⚠⚠ **Measured on THIS repo: 71 of 818 python relative specifiers resolved (8.7%) → 817 (99.9%), recovering 746 internal edges.** Everything gated on the import graph inherited it: `find_importers`, `get_blast_radius`, `get_dependency_graph`, `get_call_hierarchy` callers, `check_delete_safe`. ⚠⚠ **The consequence reproduced HERE**: `find_importers` on our own `parser/fqn.py` returned ONE importer, a test, while `tools/_utils.py` imports it in production via `from ..parser.fqn import` — a live symbol presenting as **test-only**, the exact bucket a delete-safety check calls removable. ⚠ **Gated on the IMPORTER's extension, not the specifier's shape**, so no JS/TS specifier can take the branch (`./foo`, `../lib/util`, the #284 dotted-basename convention all keep the path reading, asserted by test); Python semantics tried FIRST with fallthrough, so forms that already resolved still do. **Disclosure half**: `get_call_hierarchy` now emits `_meta.caller_graph_incomplete` naming files that call the symbol but were never considered, and refuses the absence verdict when empty. ⚠⚠ **NOT gated on `caller_count == 0`** — the report's 2nd case had 16 callers, ALL tests, production excluded; a non-zero count is exactly as incomplete and is the one nobody inspects. ⚠⚠ **Two tests failed for the RIGHT reason**: `test_find_importers`'s `python_relative` case asserted `-> None`, **encoding the defect as intended**; and `test_absence_wiring_guard` caught the new `build_verdict` site missing `index_changed` — the guard worked on the first tool to join after it was written. Tests: `test_python_relative_imports.py` (19; **7 fail pre-fix**, 8 controls pass BOTH sides).
- **Prior (1.108.253):** — **`route` answered an ambiguous question with one confident action** (follow-on to the #422 emitted-task measurement). The broad rule `/find|locate|where is|look up|search for|definition of/ -> search_symbols` fired on **26 of 40** agent-EMITTED task strings because agent phrasing almost all opens with "find", while gold split **18 `search_text` / 17 `search_symbols`** — a coin flip BY CONSTRUCTION on the majority case. ⚠⚠ **Worse than the wrong pick: a curated rule emits ONE recommendation**, so 28 of 40 had no rank 2 and `@3` was IDENTICAL to `@1` (25.0%). The caller got a coin flip with no way to see the other side. Fixed in two parts, both following the specificity block's precedence convention (narrower rules ABOVE, broad rule left INTACT so no phrasing silently loses its route): a **content-search rule** for targets that are a string not a name (quoted literals, log lines, error messages, comments, TODO/FIXME, regex, `every place`/`occurrences of`), and the broad `find` trigger now **also appends `search_text` as an alternate** — placed LAST so it can never displace an earlier rule's rank. ⚠ **HELD-OUT 20 (rows the fix was never developed against): strict `@3` 20.0% → 80.0%, crossing from 50 pts BELOW a constant answer to 10 ABOVE it**; single-recommendation cases 12/20 → 0. Human corpora do not regress (`queries.json` route@1 67.8→69.5, @3 86.4→88.1, one fewer miss; `holdout.json` unchanged 65.9/75.0). ⚠⚠ **`@1` did NOT improve and is still far below the floor** — "find X" still leads with `search_symbols` while emitted traffic skews `search_text`. That ordering was left alone DELIBERATELY: the split sits inside the labeling uncertainty the corpus author flagged, and reordering to chase it fits the sample, not the intent. A rank-1 discriminator (likely "does the task name an identifier-shaped token") needs FRESH data — do not iterate against the holdout already spent verifying this. ⚠ A rules change makes `benchmarks/route_recall/results.json` STALE; `test_retrieval_counterfactual.py` catches it (a recorded miss that now returns). Regenerate with `--write`. Tests: `test_counter.py` +9 (**9 fail pre-fix**, 2 controls pass BOTH sides).
- **Older releases (1.108.252 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **Tests:** 7211 passed, 7 skipped, **0 failed** (1.108.255, full local run POST-bump, 16m49s). ⚠ The delta from .254's 7170 is EXACTLY the two new files: 32 from `test_hook_output_channels.py` (#420) and 9 from `test_config_check_hooks.py`; nothing else moved. ⚠ **The "KNOWN 12 local-ONNX `test_semantic_search` env failures" are GONE** — .207's autouse `no_local_onnx` fixture fixed them, so a local run is now fully green and **any** red is a real signal. Do not carry that 12-failure allowance forward; it papered over a real failure once already (.197 had one hiding inside it). ⚠ **Still do not eyeball the COUNT** — diff the FAILED names against the same tree with your changes stashed; for .199 and .205 that diff was empty, and for .209 the failure set was empty outright, which is the one case that needs no baseline. ⚠ **Stashing is the wrong tool when the change is already committed and pushed** — for .205 the comparison ran in a throwaway `git worktree add --detach <pre-release-sha>`, which also survives a concurrent writer in the main tree.
- **Python:** >=3.10
