...
- **Prior (1.108.214):** — **deletion safety as PROOF OBLIGATIONS, not a score.** New `jcodemunch_mcp/investigator/` package; `investigate_deletion_safety(repo, symbol)`. A tool answers what it was asked; an investigation decides WHICH questions must be answered, asks them cheapest-first, and reports what it could not settle. ⚠⚠ **The 2026-03-18 A/B agent had every tool it needed and still got export-level liveness wrong 6/6** — it established a FILE was imported and never asked whether the EXPORT was used. **Re-measured against .213 on 2026-08-02: `check_references` answers the export question correctly and NO aggregate caller asks it.** Capability was never missing; the obligation was never raised. Five obligations, tri-state (`satisfied`/`refuted`/`unestablished`); `_verdict` is the ONLY place a conclusion is drawn and enforces **an unresolved obligation can never yield `safe`**. ⚠ **`static_clear` exists because strictness alone made the verdict USELESS** — most repos have no runtime traces, so `safe` would be unreachable and a constant verdict carries no information; obligations declare a `channel` (`static`/`dynamic`). ⚠ **The distinction it protects: `static_clear` = dynamic channel open; `not_established` = something source COULD have answered went unanswered.** Collapsing them lets a failed reference check masquerade as a clean scan. Opting out of the runtime check does NOT upgrade the verdict. ⚠ **Refutation is qualified by importer liveness** — v1 called a transitively dead symbol `unsafe` because a DEAD file imported it; breaking dead code is not breaking anything. Returns a `deletion_cluster`. Same qualifier on the text sweep or the two obligations disagree and the weaker wins. Liveness walks ONE level; an unclassifiable importer counts as REACHABLE so uncertainty blocks deletion. ⚠ **NOT registered as an MCP tool** — route@1 is 42.4%; adding a 92nd action before that moves is what .213 argues against. Tests: `test_investigator_deletion_safety.py` (19).
- **Older releases (1.108.213 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **INDEX_VERSION:** 17
