...
- **Prior (1.108.252):** — **`receipt` counted 12 of 348 calls, because it only ever looked in one profile** (#421, @MotoMato85). `_projects_root()` returned a hardcoded `~/.claude/projects`. `CLAUDE_CONFIG_DIR` relocates Claude Code's WHOLE config tree, so every session under a second profile wrote its transcript somewhere `receipt` never walked. Reporter's controlled table, same binary, only the root changed: default 12 calls / 32,646 tokens, `headroom-code-on` 223 / 745,339, `headroom-code-off` 113 / 349,280. ⚠⚠ **The single-valued `--projects-root` could NOT fix this class** — with two profiles open at once there is no one root containing both, so no invocation could ever produce a correct total. ⚠ **jMunch Console's frozen "jCode tool calls" tile is THIS bug, not a console bug** — it shells out to `receipt` without the flag and faithfully renders what it gets, while the meter-driven token tiles beside it keep moving, which is what made it read as a console defect. Fixed by learning the roots from the sessions themselves: `storage/transcript_roots.py` registers `$CLAUDE_CONFIG_DIR/projects` at server start (the client spawns us, so we INHERIT the var) and the grandparent of each hook payload's `transcript_path` as the backstop for hook-less or env-less installs; `receipt` scans the union. ⚠ **Registration is retroactive** — the moment a root is known, all history already inside it counts, so nobody has to re-run anything. ⚠⚠ **`--projects-root` stays an OVERRIDE, now repeatable** — making it additive would break the one thing it was already good for (pinning a scan) and the existing tests caught exactly that: an additive flag pulled the developer's real 12 calls into a fixture expecting 2. ⚠ Sessions dedupe by filename stem (the UUID), so a copied/symlinked tree cannot double the ledger, and a stale copy deliberately does NOT claim the stem or it would mask a fresher one. ⚠ An mtime prefilter skips files last written before `--since` (with an hour of margin) — a union multiplies the walk and Console drops its whole Savings panel to fixtures after 60s. ⚠ **The hook side effect is mute BY TEST**: Claude Code parses hook stdout as the hook's reply, so a stray write there corrupts the protocol. `receipt --roots` prints what will be walked; the registry is disclosed in SECURITY.md (directory paths only). Tests: `test_transcript_roots.py` (21).
- **Older releases (1.108.251 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **INDEX_VERSION:** 17
