"""jcm#stdio — JSON-RPC owns a private stdout; everything else goes to stderr.
...
model download emits. `tools/embed_repo.py` builds a SentenceTransformer inside a tool call.

⚠ This does NOT retire the handshake watchdog: launcher chatter written
BEFORE this process starts (the uvx case that cost a paying client 5h+)
is already in the pipe and cannot be retracted after exec.
...
def test_every_kind_of_write_is_diverted_to_stderr():
    ...
    out, err = _run("""
        import os, sys
        from jcodemunch_mcp.stdio_guard import claim_stdout
