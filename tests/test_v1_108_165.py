"""Claim-scoped evidence in the handoff contract (handoff/v2 phase 1, #377).
...
SERVED = [
    "src/runtime.py::RuntimeEvaluator.evaluate#method",
    "src/feature.py::applyExterior#method",
    "src/registry.py::Registry.register#method",
]
...
class TestClaimScopedEvidence:
            ...
            [
                {
                    ...
                    "content": "Overview.",
                    "claims": [
                        _claim("runtime.exterior", "Evaluator invokes applyExterior.", [SERVED[1]]),
                        _claim("runtime.eval", "Evaluator is the entry point.", [SERVED[0]]),
                    ],
                }
            ],
            evidence_refs=[SERVED[0]],
        )
        assert receipt["schema"] == "jcodemunch.handoff/v2"
    ...
    def test_evidence_renders_beside_the_claim(self):
            ...
            [
                {
                    ...
                    "claims": [
                        _claim(
                            "runtime.exterior",
                            "Evaluator invokes applyExterior.",
                            [SERVED[1]],
        ...
        body = handoff.get_handoff(receipt["handoff_id"])["body"]
        claim_at = body.index("### Evaluator invokes applyExterior.")
    ...
    def test_statement_preserved_verbatim(self):
        statement = "RuntimeEvaluator directly invokes `applyExterior` (see #377)."
        receipt = _finalize(
...
class TestFailsClosed:
    def test_unknown_claim_ref_names_its_claim(self):
            ...
            [
                {
                    ...
                    "claims": [
                        ...
                        _claim("c2", "Bad.", ["src/never_served.py::ghost#function"]),
                    ],
                }
            ]
        )
        ...
        assert result["invalid_claims"] == [
            {"claim_id": "c2", "unknown_refs": ["src/never_served.py::ghost#function"]}
        ]
