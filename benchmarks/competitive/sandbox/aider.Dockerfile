...
# aider.pins (108 packages, `uv pip compile --generate-hashes`). Named so GitHub's
# dependency graph does not read a competitor's sandbox-only lockfile as one of
# OUR manifests: as aider.requirements.txt it filed every advisory in aider's
# dependency tree as a Dependabot alert against this repository.
...
COPY aider.pins /opt/requirements.txt
